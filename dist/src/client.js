...
class PowerLobsterClient {
    ...
    async getRelayConfig() {
        ...
        try {
            const response = await fetch(`${RELAY_BASE_URL}/agent/${this.config.relayId}/config`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
